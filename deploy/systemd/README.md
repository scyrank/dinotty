...
2. **Or restrict the listen address** — keep `DINOTTY_BIND_ADDR=127.0.0.1` (the default) and access via an SSH tunnel or reverse proxy
...
|----------|---------|-------------|
| `DINOTTY_BIND_ADDR` | `127.0.0.1` | Listen address; LAN access requires explicitly setting `0.0.0.0` |
