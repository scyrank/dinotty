...
use super::helpers::{WORKSPACE_READ_PERMISSION, WORKSPACE_WRITE_PERMISSION};
...
fn resolve_plugin_write_path(raw: &str) -> Result<PathBuf, Response> {
    ...
    if raw_path.exists() {
        let canonical = raw_path
            .canonicalize()
            .map_err(|_| json_err(StatusCode::NOT_FOUND, "path not found"))?;
        if is_sensitive(&canonical) {
            return Err(json_err(StatusCode::FORBIDDEN, "sensitive system path"));
        }
        return Ok(canonical);
...
fn require_workspace_permission(
    ...
    let Some(plugin) = pm.registry.get(plugin_id) else {
        return Err(json_err(StatusCode::NOT_FOUND, "plugin not found"));
    };
    let permitted = plugin
        .manifest
        .permissions
        .as_deref()
        .is_some_and(|permissions| permissions.iter().any(|item| item == permission));
    if !permitted {
        return Err(json_err(StatusCode::FORBIDDEN, "plugin permission denied"));
    }
    Ok(())
}

#[allow(clippy::unused_async)]
pub async fn plugin_workspace_read_dir(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Query(q): Query<PluginPathQuery>,
) -> Response {
    try_res!(require_workspace_permission(&pm, &plugin_id, WORKSPACE_READ_PERMISSION));
...
#[allow(clippy::unused_async)]
pub async fn plugin_workspace_read_file(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Query(q): Query<PluginPathQuery>,
) -> Response {
...
#[allow(clippy::unused_async)]
pub async fn plugin_workspace_put_file(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Json(body): Json<PluginPutFileBody>,
) -> Response {
    try_res!(require_workspace_permission(&pm, &plugin_id, WORKSPACE_WRITE_PERMISSION));
...
#[allow(clippy::unused_async)]
pub async fn plugin_workspace_mkdir(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Json(body): Json<PluginMkdirBody>,
) -> Response {
    try_res!(require_workspace_permission(&pm, &plugin_id, WORKSPACE_WRITE_PERMISSION));
    let target = try_res!(resolve_plugin_write_path(&body.path));
    if target.exists() {
...
#[allow(clippy::unused_async)]
pub async fn plugin_workspace_delete(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Query(q): Query<PluginPathQuery>,
) -> Response {
...
#[allow(clippy::unused_async)]
pub async fn plugin_workspace_rename(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Json(body): Json<PluginRenameBody>,
) -> Response {
    try_res!(require_workspace_permission(&pm, &plugin_id, WORKSPACE_WRITE_PERMISSION));
...
#[allow(clippy::unused_async)]
pub async fn plugin_workspace_move(
    PathParam(plugin_id): PathParam<String>,
    State(pm): State<PluginManagerState>,
    Json(body): Json<PluginMoveBody>,
) -> Response {
    try_res!(require_workspace_permission(&pm, &plugin_id, WORKSPACE_WRITE_PERMISSION));
...
pub async fn plugin_workspace_watch(
    ...
    if let Err(response) = require_workspace_permission(&pm, &plugin_id, WORKSPACE_READ_PERMISSION)
    {
        return response;
    }
...
mod tests {
    ...
    #[test]
    fn workspace_permissions_are_enforced_server_side() {
        let manager = Arc::new(PluginManager::new("http://127.0.0.1".into(), "test".into()));
        manager.registry.insert(
            "test-plugin".into(),
            plugin_info(Some(vec![WORKSPACE_READ_PERMISSION.into()])),
        );

        assert!(require_workspace_permission(&manager, "test-plugin", WORKSPACE_READ_PERMISSION)
            .is_ok());
        assert_eq!(
            require_workspace_permission(&manager, "test-plugin", WORKSPACE_WRITE_PERMISSION)
                .unwrap_err()
                .status(),
            StatusCode::FORBIDDEN
        );
        assert_eq!(
            require_workspace_permission(&manager, "missing", WORKSPACE_READ_PERMISSION)
                .unwrap_err()
                .status(),
    ...
    async fn desktop_router_with_workspace_routes_reaches_real_handlers() {
        ...
        manager
            .registry
            .insert("p1".into(), plugin_info(Some(vec![
