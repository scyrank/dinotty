...
async fn save_settings_writes_the_token_and_load_settings_reads_it_back() {
    ...
    let persisted = std::fs::read(crate::settings::config_dir().join("settings.json")).unwrap();
    #[cfg(windows)]
    assert!(
        !String::from_utf8_lossy(&persisted).contains(PERSISTED),
        "Windows settings storage must not expose the remote token in plaintext"
    );
    let (decoded, _) = crate::platform::secret::decode_persisted(&persisted).unwrap();
    let on_disk = String::from_utf8(decoded).unwrap();
    assert!(
        on_disk.contains(PERSISTED),
        "the token must be present in the decoded settings payload, got: {on_disk}"
    );
