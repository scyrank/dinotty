...
| **Open API** | HTTP REST | 读写已有终端的字节流：屏幕、scrollback、输入、resize | [open-api.md](./open-api.md) |
| **Agent API** | HTTP + WebSocket | 命令级语义交互（执行命令、等 exit_code、订阅事件） | [agent-api.md](./agent-api.md) |
...
- **执行命令并拿 exit code** -> [POST /api/agent/run](./agent-api.md#post-apiagentrun)
- **订阅命令完成事件** -> [WS /ws/agent](./agent-api.md#websocket-接口)
...
│  - 覆盖：Tabs API、Open API、Clipboard、Settings、Plugins 等所有  │
│    /api/* 端点                                                   │
└─────────────────────────────────────────────────────────────────┘
         │
         │  Agent API 额外走 Agent Token middleware
         ▼
┌─────────────────────────────────────────────────────────────────┐
│  Agent Token (dnt_*)                                             │
│  - 通过 /api/tokens 创建的细粒度 token                            │
│  - 携带 capability: terminal:read / terminal:write 等             │
│  - 覆盖：/api/agent/*、/api/tokens/*、/mcp/*                      │
└─────────────────────────────────────────────────────────────────┘
```
...
| Open API | 需在 settings 中开启 `open_api.enabled = true` |
| Agent API | 同样需要 `open_api.enabled = true`，并使用 Agent Token |
