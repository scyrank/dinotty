...
Dinotty's external interfaces fall into four layers. This page helps you find the right API quickly.
...
| **Open API** | HTTP REST | Read and write byte streams of existing terminals: screen, scrollback, input, resize | [open-api.md](./open-api.md) |
| **Agent API** | HTTP + WebSocket | Command-level semantic interaction (run commands, await exit_code, subscribe to events) | [agent-api.md](./agent-api.md) |
...
- **Run a command and get the exit code** -> [POST /api/agent/run](./agent-api.md#post-apiagentrun)
- **Subscribe to command-finished events** -> [WS /ws/agent](./agent-api.md#websocket-endpoint)
...
Dinotty runs two parallel auth systems:
...
│    via POST /api/auth                                           │
│  - Covers: Tabs API, Open API, Clipboard, Settings, Plugins -   │
│    all /api/* endpoints                                         │
└─────────────────────────────────────────────────────────────────┘
         │
         │  Agent API additionally goes through Agent Token middleware
         ▼
┌─────────────────────────────────────────────────────────────────┐
│  Agent Token (dnt_*)                                            │
│  - Fine-grained token created via /api/tokens                   │
│  - Carries capabilities: terminal:read / terminal:write / etc.  │
│  - Covers: /api/agent/*, /api/tokens/*, /mcp/*                  │
└─────────────────────────────────────────────────────────────────┘
```
...
| Open API | Requires `open_api.enabled = true` in settings |
| Agent API | Also requires `open_api.enabled = true`, plus an Agent Token |
