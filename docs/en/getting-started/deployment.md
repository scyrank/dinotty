...
|-----------|--------|---------|-------------|
| Bind address | `--bind` / `DINOTTY_BIND_ADDR` | 127.0.0.1 | LAN listening must be enabled explicitly with `0.0.0.0`, plus a token and firewall |
| Port | `--port` / `-p` | 8999 | Server listen port |
| Token | `DINOTTY_TOKEN` env var or config file | Auto-generated | A random authentication token is generated and persisted before first startup accepts connections |
| Print token | `--print-token` | — | Decrypt and print the token as the same OS user, then exit |
...
The port default in this table applies to the standalone `dinotty-server`.
The desktop embedded service binds `127.0.0.1:0` by default, lets the OS assign
an available port, and passes that address to the bundled frontend through Tauri IPC.
Use `--embedded-port <port>` or `DINOTTY_DESKTOP_PORT` when a fixed desktop port
is required. The legacy desktop `--port/-p` option and multi-instance build port
remain compatible.
...
Tokens, `settings.json`, audit logs, and webhook secrets are stored in the
config directory. On Windows, the token and `settings.json` are encrypted with
the current user's DPAPI. Plugin persistent data lives under
`.dinotty/plugin-data` in the user's home directory.
