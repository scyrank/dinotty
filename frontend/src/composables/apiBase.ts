...
/** Legacy single-token key. Removed because local desktop tokens are memory-only. */
...
// Tauri local mode: keep the DPAPI-backed embedded token in memory only.
// Tauri remote mode: tokens entered for remote server sessions may be kept in
// the per-server map; the hub's roster credentials remain server-side.
//
...
function removeStoredLocalTokens(): void {
  try {
    ...
    if (!Object.prototype.hasOwnProperty.call(map, LOCAL_SERVER_ID)) return
    delete map[LOCAL_SERVER_ID]
    writeTokenMap(map)
  } catch {
    /* storage unavailable or malformed — it will be ignored on reads */
  }
}

// Upgrade cleanup: local desktop credentials now come only from the
// privileged, OS-protected bootstrap and must not remain in WebView storage.
removeStoredLocalTokens()
...
function readTokenMap(): Record<string, string> {
  ...
  try {
    ...
    // Local desktop credentials are obtained from the privileged Tauri
    // bootstrap and must never survive in WebView storage. Clean up both the
    // legacy flat key and any local entry created by an upstream build, while
    // leaving explicitly entered remote-server tokens scoped and persistent.
    removeStoredLocalTokens()
    delete map[LOCAL_SERVER_ID]
  } catch {
    return {}
...
export function getAuthToken(): string {
  ...
  if (activeServerId() === LOCAL_SERVER_ID) return desktopAuthToken
...
export function setAuthToken(token: string): void {
  ...
  if (activeServerId() === LOCAL_SERVER_ID) {
...
export function hasAuthToken(): boolean {
  ...
  if (id === LOCAL_SERVER_ID) return !!desktopAuthToken
